class RiskManager:
    def __init__(self, risk_per_trade: float = 0.01, risk_reward_ratio: float = 2.0, max_drawdown: float = 0.10, sl_multiplier: float = 1.5):
        """
    ...
    def calculate_sl_tp(self, side: str, current_price: float, atr: float):
        """
        Calcula el Stop Loss (SL) y Take Profit (TP) basado en la volatilidad (ATR).
        :param side: 'LONG' o 'SHORT'
        """
        if side == 'LONG':
            stop_loss = current_price - (atr * self.sl_multiplier)
